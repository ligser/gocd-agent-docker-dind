...
sh -c "$(which dind) dockerd --host=unix:///var/run/docker.sock --host=tcp://0.0.0.0:2375 --storage-driver=overlay2" > /usr/local/bin/nohup.out 2>&1 &# no arguments are passed so assume user wants to run the gocd server
